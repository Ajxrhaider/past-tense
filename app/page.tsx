...
const messagesKey = (personaId: string) => `past_tense_messages_${personaId}`;
...
export default function Home() {
  ...
  const loadMessagesForPersona = (personaId: string) => {
    const raw = localStorage.getItem(messagesKey(personaId));
    if (!raw) {
      setMessages([]);
      return;
    }
  ...
  useEffect(() => {
    ...
    loadMessagesForPersona(foundPersona.id);
    setHydrated(true);
  }, []);

  useEffect(() => {
    if (!hydrated) return;
    localStorage.setItem(messagesKey(activePersona.id), JSON.stringify(messages));
  }, [messages, activePersona.id, hydrated]);
  ...
  const handlePersonaChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    loadMessagesForPersona(selected.id);
  };

  const clearChatHistory = () => {
    if (!confirm(`Clear temporal communication history with ${activePersona.name}?`)) return;
    localStorage.removeItem(messagesKey(activePersona.id));
    setMessages([]);
  };
            ...
            >
              {categories.map((cat) => (
                <optgroup
                  key={cat}
                  label={cat}
                  style={{ backgroundColor: "#2a2a3e", color: "#6366f1", fontWeight: "bold" }}
                >
                  {personas
        ...
        {messages.length === 0 ? (
          ...
          >
            ...
            <p style={{ margin: 0, maxWidth: "480px", color: "#9ca3af", lineHeight: 1.5 }}>
              Connected to{" "}
              <span style={{ fontWeight: 600, color: "#6366f1" }}>{activePersona.name}</span> (
              {activePersona.era}). They will speak from their own worldview, controversies, and
              language.
            </p>
          </div>
        ) : (
